...
function parseColorToRgb(colorStr: string): { r: number; g: number; b: number } | null {
  ...
  const rgb = /^rgba?\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)(?:\s*,\s*[\d.]+)?\)$/i.exec(cleaned)
  if (rgb) {
    return {
  ...
  try {
    ...
    while ((styleMatch = styleRegex.exec(html)) !== null) {
      ...
      while ((ruleMatch = rulesRegex.exec(cssContent)) !== null) {
        ...
        const bgMatch = /background(?:-color)?\s*:\s*([^;]+)/i.exec(declarations)
    ...
    const elementRegex = /<(h1|h2|h3|h4|h5|h6|p|a|button|span|div|li)\b([^>]*?)>([\s\S]*?)<\/\1>/gi
    const failures: any[] = []
    let totalScanned = 0
    let match
    ...
    while ((match = elementRegex.exec(html)) !== null) {
      ...
      const inlineSizeMatch = /font-size\s*:\s*([\d.]+)(px|pt|rem|em)/i.exec(styleAttr)
      const inlineWeightMatch = /font-weight\s*:\s*(bold|[789]00)/i.test(styleAttr)
      ...
      const isLargeText = tag === "h1" || tag === "h2" || tag === "h3" || 
                          fontSizePx >= 24 || (fontSizePx >= 18.6 && isBold)
    ...
    return NextResponse.json({ error: err.message || "Failed to audit page contrast ratios" }, { status: 500 })
  }
}
