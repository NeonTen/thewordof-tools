...
export async function POST(req: Request) {
  try {
    ...
    if (Array.isArray(protectedKeywords) && protectedKeywords.length > 0) {
      rules.push(
        `- CRITICAL RULE: You MUST preserve the following terms exactly as they are without any modifications, simplification, or substitution: ${protectedKeywords.map((k) => `"${k}"`).join(", ")}. Do NOT rewrite or simplify these words/phrases.`,
      );
    }
