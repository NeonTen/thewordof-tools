...
export function UpgradeButton({ user, className, children }: UpgradeButtonProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger 
        render={<Button className={cn("w-full h-12 font-bold", className)} />}
      >
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Select your preferred way to pay for TheWordOf Tools Pro.
          </DialogDescription>
        </DialogHeader>
            ...
            <PayPalButtons 
              ...
              createOrder={async () => {
                const res = await fetch("/api/paypal/create-order", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ amount: "5.99", currency: "USD" })
                })
